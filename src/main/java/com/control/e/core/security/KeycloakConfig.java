...
//        resourceSecurityConfigs.forEach(resSec -> resSec.configure(http));
        // FIXME Remover após migração do Angular 7 para 11
        http.authorizeRequests()
                .regexMatchers(HttpMethod.GET,STATIC_RESOURCES).permitAll()
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        frontend.setDirectAccessGrantsEnabled(true);
    ...
    private void createRootUser() {
        if (Arrays.asList(env.getActiveProfiles()).isEmpty()) {
            ...
            userRepresentation.setEmail("root@email.br");
